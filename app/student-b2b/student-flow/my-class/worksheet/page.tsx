...
export default function MyClassAssesmentPage() {
	...
	const [activeContentTab, setActiveContentTab] = useState(contentTabsData[3]); // Default to Learning
    ...
    const renderActiveContent = () => {
        switch (activeContentTab) {
            case 'Learning':
                return <div className="space-y-3">{learningWeeksDataPage.map(week => <LearningAccordion key={week.id} week={week} isOpen={openAccordionId === week.id} onToggle={() => handleAccordionToggle(week.id)} />)}</div>;
            case 'Assessments':
                return <div className="space-y-3">{assessmentDataPage.map(assessment => <AssessmentItem key={assessment.id} assessment={assessment} />)}</div>;
            case 'Mock Papers':
                return <div className="space-y-3">{mockPapersDataPage.map(item => <MockPaperItem key={item.id} item={item} />)}</div>;
            case 'Work Sheet':
                return <div className="space-y-3">{workSheetDataPage.map(item => <WorkSheetItem key={item.id} item={item} />)}</div>;
            default:
                return <div className="text-center py-10 text-gray-500">{activeContentTab} content goes here.</div>;
        }
    };
		...
		<div className="bg-gray-100 min-h-screen flex flex-col">
			...
			<main className="flex-grow container mx-auto p-3 sm:p-4 md:p-6 lg:p-8">
				...
				<div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
					...
					<div className="md:col-span-3 lg:col-span-4"> {/* Content takes remaining cols */}
                        <ContentDisplayArea
