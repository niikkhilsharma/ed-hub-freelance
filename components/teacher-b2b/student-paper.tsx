...
const StarRatingDisplay: React.FC<{ currentRating: number; maxRating: number }> = ({ currentRating, maxRating }) => {
	return (
...
const SummaryPanel: React.FC<{ summary: AssessmentSummaryData }> = ({ summary }) => {
	...
	return (
		<div className="bg-white rounded-2xl space-y-5">
			...
			<button
				className={`w-full flex items-center justify-center gap-2 py-1.5 ${BACKGROUND_LIGHT_GRAY_BOX} rounded-xl hover:bg-gray-200 transition-colors`}>
				<IoTimeOutline className={`w-5 h-5 ${COLOR_RATING_STARS}`} />
				<span className={`text-lg  ${COLOR_RATING_STARS}`}>View Detailed Report</span>
			</button>

			{/* Ratings Section */}
			<div className="space-y-4 mt-6 pt-1">
				{' '}
	...
	return (
		<div className="bg-[#eeeeee] min-h-screen flex flex-col">
			...
			<div className="flex items-center gap-2 bg-white px-6 py-4">
				...
				<h1 className="text-xl font-bold text-[#FF3366]">Student Name</h1> {/* Or dynamic course name */}
			</div>

			<main className="flex-grow w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-8">
