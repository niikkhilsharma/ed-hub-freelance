...
const TeacherListItem = ({
  ...
  <button
    onClick={onClick}
    className={`w-full flex items-center p-3 rounded-2xl transition-colors ${isActive ? "bg-[#FFCC0026]" : "hover:bg-gray-100"
      }`}
  >
    <Image
...
const ChatMessage = ({ message }: { message: ChatMessageData }) => {
  ...
  return (
    <div className={`flex mb-4 ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-xs lg:max-w-md w-full px-4 py-3 rounded-2xl   ${isUser ? "bg-[#3366FF] text-white " : "bg-[#F9FAFB] text-black"
          }`}
      >
        {message.text && (
