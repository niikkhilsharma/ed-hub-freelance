    ...
    <div className="bg-[#eeeeee] min-h-screen flex flex-col">
      ...
      <div className="bg-white ">
        <div className="flex max-w-[96rem] mx-auto items-center gap-2 px-6 py-4">
          ...
          <h1 className="text-lg font-bold text-[#FF3366]">Test Name</h1>{" "}
...
const GeneralFilterButton: React.FC<{
  ...
  onSelect: (option: string, filterId: string) => void;
}> = ({ filter, onSelect }) => {
  ...
  return (
    <div className="relative flex-shrink-0" ref={dropdownRef}>
      <button
        ...
        className="flex items-center justify-center gap-1.5 px-3 py-1.5 sm:px-3.5 sm:py-2.5 border border-gray-300 bg-[#F9FAFB] text-black rounded-xl text-xs whitespace-nowrap hover:bg-gray-100 transition-colors"
      >
      ...
      {isOpen && (
        <div className="absolute left-0 mt-1 w-36 bg-white border rounded-xl shadow-lg z-50 text-xs overflow-hidden">
          <button
            onClick={() => {
              onSelect("Option 1", filter.id);
              setIsOpen(false);
            }}
          ...
          {sampleGeneralFilters.map((filter) => (
            <GeneralFilterButton
              ...
              onSelect={(option, filterId) =>
                alert(`Selected ${option} from ${filter.label} (${filterId})`)
              }
            />
          ))}
        </div>
