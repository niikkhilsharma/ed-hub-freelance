...
const CHAPTER_ACCORDION_BG = "bg-gray-50"; // Background for chapter accordions
const INFO_PANEL_BG = "bg-slate-50"; // Background for the right info panel
...
const samplePedagogyInfo: PedagogyInfo = {
  learningObjectives: [
    ...
    { id: 'lo2', level: 'PINK:', points: [ /* ... similar points ... */ ]},
    { id: 'lo3', level: 'PURPLE:', points: [ /* ... similar points ... */ ]},
  ],
  materialsRequired: [
...
const TopTabButton: React.FC<{ tab: TopCategoryTab; isActive: boolean; onClick: () => void; }> = ({ tab, isActive, onClick }) => (
  <button
    onClick={onClick}
...
const ChapterSubTabButton: React.FC<{ tab: ChapterSubTab; isActive: boolean; onClick: () => void; }> = ({ tab, isActive, onClick }) => (
  <button
    onClick={onClick}
    className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-all duration-150 whitespace-nowrap focus:outline-none
      ${isActive ? `text-[${PRIMARY_BLUE}] border-b-2 border-[${PRIMARY_BLUE}]` : 'text-gray-600 hover:text-gray-800 border-b-2 border-transparent hover:border-gray-300'}`}
  >
    {tab.name}
  </button>
);

const ChapterAccordion: React.FC<{ item: ChapterAccordionItem; isOpen: boolean; onToggle: () => void; }> = ({ item, isOpen, onToggle }) => (
  <button
    onClick={onToggle}
    className={`w-full flex justify-between items-center p-3.5 ${CHAPTER_ACCORDION_BG} hover:bg-gray-100 rounded-lg border border-gray-200 shadow-sm transition-colors`}
  >
...
const InfoPanel: React.FC<{ info: PedagogyInfo }> = ({ info }) => (
  <div className={`${INFO_PANEL_BG} p-4 sm:p-5 rounded-xl shadow-lg space-y-6 h-full overflow-y-auto custom-scrollbar-thin ${SCROLLBAR_THUMB_ORANGE} ${SCROLLBAR_TRACK_LIGHT}`}>
    <div>
      <h3 className={`text-base font-semibold text-[${PRIMARY_BLUE}] mb-2.5`}>Learning Objectives</h3>
      <div className="space-y-3 text-xs text-gray-700">
        {info.learningObjectives.map(obj => (
          <div key={obj.id}>
            <p className="font-medium text-gray-800 mb-1">{obj.level}</p>
            <ul className="list-disc list-outside pl-4 space-y-0.5">
              {obj.points.map((point, idx) => <li key={idx}>{point}</li>)}
            </ul>
          </div>
        ))}
      </div>
    ...
    <div>
      <h3 className={`text-base font-semibold text-[${PRIMARY_BLUE}] mb-2.5`}>Material required:</h3>
      <ul className="list-disc list-outside pl-4 space-y-1 text-xs text-gray-700">
        {info.materialsRequired.map(mat => <li key={mat.id}>{mat.text}</li>)}
      </ul>
    </div>
  </div>
);
...
const PedagogyContent: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-0 sm:p-0 lg:p-0 overflow-hidden"> {/* Full width container for layout */}
        {/* Top Categories */}
        <div className="bg-white px-4 sm:px-6 py-3 border-b border-gray-200">
            <nav className="flex space-x-1 sm:space-x-2 overflow-x-auto custom-scrollbar-thin pb-1 justify-center sm:justify-start">
        ...
        {/* Main Content Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 items-start"> {/* No gap, info panel is distinct */}
            {/* Left Column: Chapters and Accordions */}
            <div className="lg:col-span-2 p-4 sm:p-6 lg:p-8 space-y-6 border-r-0 lg:border-r border-gray-200 min-h-[calc(100vh-250px)]"> {/* Adjust min-h */}
                {/* Chapter Sub-Tabs */}
                {chapterTabsForActiveCategory.length > 0 && (
                    <div className="pb-3 border-b border-gray-200">
                        ...
                        {chapterTabsForActiveCategory.map(chTab => (
                            ...
                            onClick={() => { setActiveChapterId(chTab.id); setOpenAccordionId(null); }}
                            />
                        ))}
                        </nav>
                    </div>
                ...
                {accordionsForActiveChapter.length > 0 ? (
                    <div className={`space-y-3 max-h-[calc(100vh-400px)] overflow-y-auto pr-2 ${SCROLLBAR_THUMB_ORANGE} ${SCROLLBAR_TRACK_LIGHT} scrollbar-thin`}> {/* Adjust max-h */}
            ...
            <div className="lg:col-span-1 p-4 sm:p-6 lg:p-6"> {/* Consistent padding */}
    ...
    <div className="bg-[#eeeeee] min-h-screen flex flex-col">
      ...
      <div className="bg-white px-4 sm:px-6 py-3 shadow-sm sticky top-0 z-40">
        <div className="container mx-auto flex items-center gap-3">
            <button onClick={handleBackClick} className="p-1.5 text-gray-700 hover:text-gray-900 focus:outline-none rounded-md" aria-label="Go back">
