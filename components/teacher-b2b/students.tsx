...
const AVATAR_PLACEHOLDER =
  "https://picsum.photos/seed/studentlist/40/40?grayscale";
...
const StudentItemRow: React.FC<{
  ...
  <div
    className={`${STUDENT_ITEM_BG} border border-[#B0B0B0] rounded-2xl p-3 sm:p-3.5 shadow-sm hover:shadow-md transition-shadow duration-200 flex items-center justify-between gap-3`}
  >
    <div className="flex items-center gap-3 min-w-0">
      <img
        src={student.avatarUrl}
        alt={student.name}
        className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl object-cover flex-shrink-0"
      />
      <span className="text-sm text-gray-800 font-medium truncate self-start">
        {student.name}
      </span>
    </div>
