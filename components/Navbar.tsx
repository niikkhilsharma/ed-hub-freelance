...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full h-[120px] z-50 flex flex-col">
      ...
      <div className="flex-2 bg-white text-black min-h-[80px] px-[8vw] py-2 flex items-center justify-between relative">
        ...
        <div className="ml-16 w-full justify-between hidden md:flex items-center">
          {["/", "/about", "/courses", "/future-school", "/subscription", "/contact"].map(
            (href, idx) => (
              <Link
                key={href}
                href={href}
          ...
          <button className="px-4 py-1.5 text-sm text-white rounded-sm bg-[#3366ff] hover:bg-blue-700 cursor-pointer">
            Login
          </button>
        </div>
        ...
        {isMenuOpen && (
          <div className="absolute top-full left-0 w-full bg-white flex flex-col gap-4 px-8 py-6 shadow-lg md:hidden">
            ...
            <button className="px-4 py-2 text-sm text-white rounded-sm bg-[#3366ff] hover:bg-blue-700">
              Login
            </button>
          </div>
        )}
