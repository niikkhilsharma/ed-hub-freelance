...
const AccordionItem = ({ unit, isOpen, onToggle }: { unit: Unit; isOpen: boolean; onToggle: () => void }) => (
	<div
		className={cn('rounded-2xl bg-white p-4', !isOpen && 'bg-white border border-[#E5E7EB]')}
		style={
			!isOpen
				? {}
				: {
		...
		<button
			...
			className={`w-full flex justify-between items-center${
				isOpen ? 'bg-white rounded-2xl mb-4' : 'hover:bg-gray-100 rounded-[25px]'
			}`}>
			<span className="text-xl text-black">{unit.name}</span>
			<div className="flex items-center gap-5">
				{isOpen &&
					unit.periods !== undefined && ( // Show periods/marks in open state header as per image
						<span className="text-[14px] text-[#6B7280]">Periods : {unit.periods}</span>
					)}
				{isOpen &&
					unit.marks !== undefined && ( // Show periods/marks in open state header as per image
		...
		<div
			className={`rounded-[25px] z-20 overflow-hidden shadow-sm ${
				isOpen ? 'bg-transparent border-0' : 'bg-white' // CSS colors
			}`}>
			{isOpen && (
				<div className="relative rounded-b-[25px]">
					{/* Outer container for pattern + content box */}
					{/* {unit.patternSrc && (
					<div className="absolute inset-0 z-0 rounded-b-[25px] overflow-hidden">
						<Image src={unit.patternSrc} alt="Unit background pattern" layout="fill" objectFit="cover" />
						<div className="absolute inset-0 bg-black/20 z-10"></div>
					</div>
				)} */}
					{/* White content box on top of pattern */}
					<div className="relative z-20 p-5 bg-white rounded-[25px] mb-5">
						{' '}
						{/* CSS shows SubDetails as a separate white box */}
						<h3 className="text-[16px] font-normal text-[#6B7280] mb-3">
							{' '}
							{/* Poppins 400, #6B7280, size 16px */}
							{unit.details[0]?.point}
						</h3>
						<ol className="list-decimal list-inside space-y-1 text-[16px] text-[#6B7280] leading-[24px] ml-4">
							{' '}
							{/* line-height 24px */}
...
const modulesData: Unit[] = Array.from({ length: 8 }, (_, i) => ({
	...
	details: i === 0 ? unitDetailsSample : [{ point: `Content for Unit Name ${i + 1}` }],
}))
...
const totalInternalMarks =
	internalAssessmentData.reduce((sum, item) => sum + parseInt(item.marks.split(' ')[0]), 0) + ' Marks'
	...
	return (
		<div className="bg-[#EEEEEE] min-h-screen flex flex-col">
			...
			<div className="flex items-center gap-2 px-6 py-4 bg-white">
				...
				<h1 className="text-xl font-semibold text-[#FF3366]">Yearly Plan Overview</h1>
			</div>
			...
			<div className="w-full bg-[#f1f1f1] py-4 px-10">
                ...
                ${activeSubject === subject ? 'bg-[#FF3366] text-white shadow-sm' : 'text-slate-700 hover:text-pink-500'}`}>
							{subject}
						</button>
					))}
				</div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
					...
					<div className="lg:col-span-1 space-y-6">
						...
						<div className="bg-white rounded-[25px] shadow-lg p-6">
							<h3 className="text-[18px] font-semibold text-[#3366FF] text-center mb-4">COURSE STRUCTURE CLASS - IX</h3>
							<table className="w-full text-[16px] border border-[#E5E7EB]">
								<thead className="bg-[#E0F2FE]">
									<tr>
										<th className="px-3 py-2 border border-[#E5E7EB] text-left font-medium text-[#1E1E1E]">Units</th>
										<th className="px-3 py-2 border border-[#E5E7EB] text-left font-medium text-[#1E1E1E]">Unit Name</th>
										<th className="px-3 py-2 border border-[#E5E7EB] text-left font-medium text-[#1E1E1E]">Marks</th>
									</tr>
								</thead>
								<tbody>
									{courseStructureData.map(item => (
										<tr key={item.unit} className="bg-white">
											<td className="px-3 py-2 border border-[#E5E7EB] text-[#1E1E1E]">{item.unit}</td>
											<td className="px-3 py-2 border border-[#E5E7EB] text-[#1E1E1E]">{item.name}</td>
											<td className="px-3 py-2 border border-[#E5E7EB] text-[#1E1E1E] font-medium">{item.marks}</td>
										</tr>
									))}
									<tr className="bg-[#E0F2FE] font-semibold">
										...
										<td className="px-3 py-2 border border-[#E5E7EB] text-[#1E1E1E]">{totalCourseMarks}</td>
									</tr>
								</tbody>
							</table>
						</div>
						...
						<div className="bg-white rounded-[25px] shadow-lg p-6">
							<h3 className="text-[18px] font-semibold text-[#3366FF] text-center mb-1">MATHEMATICS</h3>
							<h4 className="text-[16px] font-medium text-[#1E1E1E] text-center mb-1">QUESTION PAPER DESIGN</h4>
							<h4 className="text-[16px] font-medium text-[#1E1E1E] text-center mb-4">CLASS - IX</h4>
							...
							<table className="w-full text-[14px] leading-relaxed border border-[#E5E7EB]">
								<thead className="bg-[#E0F2FE]">
									<tr>
										<th className="px-2 py-1 border border-[#E5E7EB] text-left font-medium text-[#1E1E1E]">S. No.</th>
										<th className="px-2 py-1 border border-[#E5E7EB] text-left font-medium text-[#1E1E1E]">Typology of Questions</th>
										<th className="px-2 py-1 border border-[#E5E7EB] text-left font-medium text-[#1E1E1E] text-center">Total Marks</th>
										<th className="px-2 py-1 border border-[#E5E7EB] text-left font-medium text-[#1E1E1E] text-center">% Weightage</th>
									</tr>
								</thead>
								<tbody>
									{questionPaperDesignData.map(item => (
										<tr key={item.sno} className="bg-white">
											<td className="px-2 py-1 border border-[#E5E7EB] text-[#1E1E1E] align-top">{item.sno}</td>
											<td className="px-2 py-1 border border-[#E5E7EB] text-[#1E1E1E] align-top">{item.typology}</td>
									...
									<tr className="bg-[#E0F2FE] font-semibold">
										...
										<td className="px-2 py-1 border border-[#E5E7EB] text-[#1E1E1E] text-center">{totalDesignMarks}</td>
										<td className="px-2 py-1 border border-[#E5E7EB] text-[#1E1E1E] text-center">{totalDesignWeightage}</td>
									</tr>
								</tbody>
							</table>

							<h3 className="text-[18px] font-semibold text-[#3366FF] text-center mt-6 mb-2">INTERNAL ASSESSMENT</h3>
							<table className="w-full text-[16px] border border-[#E5E7EB]">
								<thead className="bg-[#E0F2FE]">
									<tr>
										<th className="px-2 py-1 border border-[#E5E7EB] text-left font-medium text-[#1E1E1E] w-3/4"></th>
										<th className="px-2 py-1 border border-[#E5E7EB] text-left font-medium text-[#1E1E1E]">Marks</th>
									</tr>
								</thead>
								<tbody>
									{internalAssessmentData.map(item => (
										<tr key={item.item} className="bg-white">
											<td className="px-2 py-1 border border-[#E5E7EB] text-[#1E1E1E]">{item.item}</td>
											<td className="px-2 py-1 border border-[#E5E7EB] text-[#1E1E1E] font-medium text-center">{item.marks}</td>
										</tr>
									))}
									<tr className="bg-[#E0F2FE] font-semibold">
										<td className="px-2 py-1 border border-[#E5E7EB] text-[#1E1E1E]">Total</td>
										<td className="px-2 py-1 border border-[#E5E7EB] text-[#1E1E1E] text-center">{totalInternalMarks}</td>
									</tr>
								</tbody>
							</table>
						</div>
