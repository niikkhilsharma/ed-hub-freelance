        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[100] p-4 transition-opacity duration-300 ease-in-out">
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all duration-300 ease-in-out scale-100 text-center p-6 md:p-8 relative">
                ...
                <div className="mb-6 flex justify-center">
                    <Image
                        src="/placeholder-error-icon.png" // <-- UPDATE PATH to your error icon image
                        alt="System Error"
...
interface Question { id: number; text: string; options: QuestionOption[]; correctOptionId?: string; }

const sampleQuestions: Question[] = [ /* ... your questions ... */ ];
...
export default function DmittTestPageWithErrorModal() {
    ...
    const formatTime = (seconds: number) => { /* ... formatTime logic ... */ return `${Math.floor(seconds / 60).toString().padStart(2, '0')}:${(seconds % 60).toString().padStart(2, '0')}`; };
    const currentQuestion = sampleQuestions[currentQuestionIndex];
    const handleOptionSelect = (optionId: string) => { /* ... option select logic ... */ setSelectedOptionId(optionId); };
    const handleNextQuestion = () => { /* ... next question logic ... */
    ...
    return (
        <Fragment> {/* Needed for the modal to be a sibling */}
            <div className={`min-h-screen bg-gray-100 flex flex-col ${isErrorModalOpen ? 'filter blur-sm pointer-events-none' : ''}`}> {/* Blur background when modal is open */}
                {/* Header (from previous) */}
                <header className="bg-blue-600 text-white shadow-md sticky top-0 z-40"> {/* Lower z-index than modal */}
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex justify-between items-center">
                        <span className="text-2xl font-bold italic">EDUNIQUE</span>
                        <div className="flex items-center space-x-4">
                            <button className="p-1.5 rounded-full hover:bg-blue-700 focus:outline-none"><FiBell className="w-5 h-5" /></button>
                            <div className="flex items-center space-x-2">
                                <Image src="/placeholder-avatar.jpg" alt="Shlok Agheda" width={32} height={32} className="rounded-full" />
                                <div><p className="text-sm font-medium">Shlok Agheda</p><p className="text-xs opacity-80">Student</p></div>
                            </div>
                        </div>
                    </div>
                </header>
                    ...
                    <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-xl p-6 md:p-8">
                        ...
                        <div className="flex space-x-3 border-b border-gray-200 mb-8 overflow-x-auto pb-2">
                            {tabCategories.map(category => (
                                <button key={category} onClick={() => setActiveCategory(category)}
                                    className={`px-4 py-2.5 text-sm font-medium rounded-t-lg whitespace-nowrap focus:outline-none transition-colors duration-150 ${activeCategory === category ? 'bg-pink-500 text-white' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}>
                                    {category}
                                </button>
                            ))}
                        </div>
                        <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
                            <div>
                                ...
                                <div className="mt-2 inline-flex items-center justify-center px-3 py-1 bg-green-100 text-green-700 text-xs font-semibold rounded-full">{currentQuestionIndex + 1} / {TOTAL_QUESTIONS}</div>
                            </div>
                            <div className="text-right">
                                <div className="flex items-center gap-1.5 text-pink-500"><FiClock className="w-5 h-5" /><span className="text-2xl font-bold">{formatTime(timeLeft)}</span></div>
                                ...
                                <div className="space-y-3">
                                    {currentQuestion.options.map(option => (
                                        <button key={option.id} onClick={() => handleOptionSelect(option.id)}
                                            className={`w-full text-left flex items-center p-3.5 border-2 rounded-xl transition-all duration-150 focus:outline-none ${selectedOptionId === option.id ? 'bg-blue-100 border-blue-500 ring-1 ring-blue-500' : 'bg-white border-gray-200 hover:border-gray-300 hover:bg-gray-50'}`}>
                        ...
                        <div className="mt-10 flex justify-end">
                            <button onClick={handleNextQuestion} disabled={!selectedOptionId && currentQuestionIndex < TOTAL_QUESTIONS -1}
