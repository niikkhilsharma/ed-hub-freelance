...
const InfoBadge = ({ icon: Icon, text, bgColor, textColor }: { icon: React.ElementType, text: string, bgColor: string, textColor: string }) => (
...
const AssessmentStatus = ({ statusText, icon: Icon, iconColor, progressPercentage, timeLeft }: { statusText: string, icon: React.ElementType, iconColor: string, progressPercentage: number, timeLeft: string }) => (
    <div className="bg-pink-50 p-6 rounded-xl mt-8"> {/* Light pink background */}
        <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2">
                <span className="text-md font-semibold text-pink-600">{statusText}</span>
                <Icon className={`w-5 h-5 ${iconColor}`} />
            </div>
            <span className="text-xs font-medium text-pink-600">{progressPercentage}% complete</span>
        </div>
        <div className="w-full bg-pink-200 rounded-full h-2.5 mb-2">
            <div
                className="bg-pink-500 h-2.5 rounded-full"
                style={{ width: `${progressPercentage}%` }}
            ></div>
        </div>
        <p className="text-xs text-gray-600 text-center">{timeLeft} hrs left</p>
    </div>
);
        ...
        >
            <div className="bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row w-full max-w-4xl lg:max-w-5xl">
                ...
                <div className="w-full md:w-3/5 bg-white p-6 sm:p-8 lg:p-10 flex flex-col">
                    ...
                    <div className="bg-gray-50 p-6 rounded-xl mb-6 space-y-3"> {/* Reduced mb from mb-8 to mb-6 */}
                ...
                <div className="w-full md:w-2/5 relative min-h-[350px] md:min-h-full">
                    ...
                    <div className="relative z-10 w-full h-full flex items-center justify-center p-4 md:p-0">
                        <Image
                            src="/images/student-with-laptop.png" // <-- UPDATE PATH
