...
const Navbar = ({ user, activeState }: NavbarProps) => {
  const [activeTab, setActiveTab] = useState(activeState || "dashboard");
  ...
  // Sync state when activeState prop changes
  useEffect(() => {
    setActiveTab(activeState || "dashboard");
  }, [activeState]);
    ...
    <div className="bg-[#3366ff]">
      <nav className="text-white px-2 py-2 sm:px-4 max-w-[96rem] mx-auto sm:py-6 relative">
        <div className="flex items-center gap-4 justify-between">
          ...
          {/* Desktop Nav */}
          <div className="bg-[#e3f2fd]/15 rounded-full px-3 sm:px-4 md:px-6 py-2 sm:py-3 md:py-4 hidden sm:flex items-center justify-center gap-4 sm:gap-5 md:gap-6 overflow-x-auto max-w-full">
            {navItems.map((item) => (
              <button
                key={item.key}
                onClick={() => setActiveTab(item.key)}
                className={`flex items-center gap-2 text-sm sm:text-md transition-all whitespace-nowrap ${
                  activeTab === item.key
